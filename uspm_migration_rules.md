...
资源迁移 (UCloud Resource Migration) 提供云服务资源的迁移以及整合服务。以下内容适用于【同项目资源迁移】。
...
| 产品类型 | 要求 | 迁移规则 |
| --- | --- | --- |
| 云主机 UHost | 未过期 <br> 未开启磁盘加密 <br> 未开启数据方舟 <br> 未开启硬件隔离 <br> 未绑定负载均衡ULB <br> 非后付费资源 <br> 非大数据型D机型 <br> 非私有专区资源 <br> 非容器云主机| 迁移前后，云主机的资源ID、MAC地址均不变。内网IP根据自定义配置决定是否要改变。<br> 注：仅支持已在镜像内部预装了cloud-init组件的云主机进行自定义内网IP配置。若云主机未预装cloud-init，则在完成迁移操作后，其内网IP地址将维持原状，不做更改。 |
...
- 迁移开始前，请检查待迁移资源未过期
- 迁移过程中，资源无法使用，同时计费不停止
- 迁移结束后，迁入端资源继续保持迁出端资源的计费周期
